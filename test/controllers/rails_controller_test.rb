...
class RailsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create rail" do
    assert_difference('Rail.count') do
      post rails_url, params: { rail: { User: @rail.User, email: @rail.email, name: @rail.name } }
    end
  ...
  test "should update rail" do
    patch rail_url(@rail), params: { rail: { User: @rail.User, email: @rail.email, name: @rail.name } }
